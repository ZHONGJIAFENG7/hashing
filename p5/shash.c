...
#define PRIME 31
#define RANGE 0.6
static void check_size(shash_t *ptr);
...
int shash_search(shash_t * ptr, char *str, int *count);
...
static void check_size(shash_t *ptr) {
   ...
   if((double)ptr->length/(double)ptr->size> RANGE) {
      ...
      shash_t *nptr = shash_create(newsize);
...
shash_t *shash_create(int size) {
   int i = 0;
   shash_t * ptr = (shash_t *) malloc(sizeof(shash_t) * 1) ;
